...
This backend has been converted from the original Express/Mongoose API to Java Spring Boot with PostgreSQL.
...
Set the values from `.env.example` as environment variables before running. Spring Boot reads environment variables directly; it does not load `.env` files by itself.

Default database settings:

```env
POSTGRES_URL=jdbc:postgresql://localhost:5432/uber_clone
...
The REST route paths remain compatible with the MERN backend: `/users`, `/captains`, `/maps`, and `/rides`.

## Geospatial Hexagonal Architecture

Captain location indexing is organized as a hexagonal architecture slice:

- Application port: `application/port/CaptainLocationIndexPort`
- Application use case: `application/service/CaptainLocationService`
- H3 adapter: `infrastructure/geo/H3GeoIndexAdapter`
- Postgres adapter: `infrastructure/persistence/PostgresCaptainLocationIndexAdapter`

When a captain emits `update-location-captain`, the backend stores both the raw latitude/longitude and the H3 cell in `captain.h3Index`. Ride creation converts the pickup coordinate into nearby H3 cells, queries captains by indexed `h3Index`, then applies an exact distance filter before sending `new-ride`.

This is not a square-to-hex conversion. H3 converts latitude/longitude coordinates into hierarchical hexagonal cell indexes.
